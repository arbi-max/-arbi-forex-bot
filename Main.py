...
if upcoming_high:
    n = upcoming_high[0]
    return {"status": "WAIT", "reason": "News ROUGE dans " + str(n["mins"]) + "min : " + n["title"], "action": "Attendre la sortie puis trader la reaction", "events": upcoming_high}

if just_released:
    n = just_released[0]
    return {"status": "TRADE_AFTER", "reason": "News ROUGE sortie il y a " + str(abs(n["mins"])) + "min : " + n["title"], "action": "Trader la reaction - confirmer sur M15", "events": just_released}

if recent_high:
    n = recent_high[0]
    return {"status": "TRADE_AFTER", "reason": "Post-news ROUGE (" + str(abs(n["mins"])) + "min) : " + n["title"], "action": "Marche en digestion - confirmer tendance", "events": recent_high}

if upcoming_medium:
    n = upcoming_medium[0]
    return {"status": "CAUTION", "reason": "News ORANGE dans " + str(n["mins"]) + "min : " + n["title"], "action": "Trade possible mais lot reduit 50%", "events": upcoming_medium}

return {"status": "CLEAR", "reason": "Aucune news impactante", "action": "Trading normal", "events": []}
```
...
if news_ctx["status"] == "TRADE_AFTER":
    bull_score += 1
    bull_reasons.append("Post-news : opportunite de trade")
...
if news_ctx["status"] == "TRADE_AFTER":
    bear_score += 1
    bear_reasons.append("Post-news : opportunite de trade")
...
msg_start += “Niveau : Institutionnel SMC\n”
msg_start += “Paires : EUR/USD | GBP/USD | USD/JPY | XAU/USD\n”
msg_start += “Analyse : D1 > H4 > H1 > M15\n”
msg_start += “News : Logique Pro (trade la reaction)\n”
msg_start += “Seuil signal : 10/30 confluences\n”
msg_start += “Capital : “ + str(CAPITAL) + “$ | Risque : “ + str(RISK_PCT) + “%\n”
msg_start += “En surveillance… Les signaux arrivent bientot”
send(msg_start)
...
time.sleep(2)
maybe_reset_signals()
print(”[” + datetime.now().strftime(”%H:%M”) + “] Cycle termine - prochain scan dans 5min”)
