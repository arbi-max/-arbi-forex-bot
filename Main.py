...
for e in events:
    ...
    try:
        ...
        if impact == "High":
            if 0 < diff <= 45:
                upcoming_high.append({"title": title, "country": country, "mins": int(diff)})
            elif -15 <= diff <= 0:
                just_released.append({"title": title, "country": country, "mins": int(diff)})
            elif -60 <= diff < -15:
                recent_high.append({"title": title, "country": country, "mins": int(diff)})
        elif impact == "Medium":
            if 0 < diff <= 20:
                upcoming_medium.append({"title": title, "country": country, "mins": int(diff)})
    except:
        pass
